...
const fmtDate = (d: string | null) => {
  if (!d) return 'Never';
  try {
    return new Intl.DateTimeFormat('en-MY', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(d));
  } catch (e) {
    return 'Invalid Date';
  }
};
...
function RoleBadge({ role }: { role: UserRole }) {
  const safeRole = ROLE_STYLE[role] || 'bg-gray-100 text-gray-600 border-gray-200';
  return (
    <span className={cn('inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full border text-xs font-medium', safeRole)}>
      <Shield className="w-3 h-3" />{role || 'Unknown'}
    </span>
  );
}
...
function ActionMenu({ user, currentUser, onEdit, onDelete, onToggle, onReset, onHistory }: {
  ...
  const { isManager, isSupervisor } = usePermission(currentUser?.role || 'cashier');
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-52 bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-xl shadow-xl z-30 overflow-hidden py-1">
    ...
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Staff Member' : 'Add Staff Member'}>
      <form onSubmit={submit} className="space-y-4">
        ...
        <FormField label="Role">
          ...
          <div className="mt-2 p-3 bg-gray-50 dark:bg-neutral-800/50 rounded-xl border border-gray-100 dark:border-neutral-800 space-y-1.5">
            <p className="text-xs font-semibold text-gray-500 dark:text-neutral-400">Permissions:</p>
            {ROLE_PERMS[role]?.map(p => (
              <p key={p} className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-neutral-300">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />{p}
              </p>
            )) || <p className="text-xs text-gray-500 italic">No permissions defined.</p>}
          </div>
        </FormField>
        ...
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={onClose}
            className="flex-1 py-2.5 border border-gray-200 dark:border-neutral-700 rounded-xl text-sm font-medium text-gray-600 dark:text-neutral-300 hover:bg-gray-50 dark:hover:bg-neutral-800 transition-all">
            Cancel
          </button>
        ...
        <div className="space-y-2">
          {history.map(e => (
            <div key={e.id} className={cn(
              ...
              e.success ? 'bg-gray-50 border-gray-100 dark:bg-neutral-800/50 dark:border-neutral-700' : 'bg-red-50 border-red-100 dark:bg-red-900/20 dark:border-red-900/50'
            )}>
              <div className="flex items-center gap-3">
                <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center shrink-0',
                  e.success ? 'bg-emerald-100 dark:bg-emerald-500/20' : 'bg-red-100 dark:bg-red-500/20')}>
                  <LogIn className={cn('w-4 h-4', e.success ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400')} />
                </div>
                <div>
                  <p className="font-medium text-gray-800 dark:text-neutral-200">{e.success ? 'Signed in successfully' : 'Failed attempt'}</p>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {MATRIX_ROWS.map((row, i) => (
              <tr key={i} className="border-b border-gray-50 dark:border-neutral-800 hover:bg-gray-50/50 dark:hover:bg-neutral-800/50 transition-colors">
                <td className="py-2.5 pr-4 text-gray-700 dark:text-neutral-300">{row.page}</td>
                {[row.m, row.s, row.c].map((has, j) => (
                  <td key={j} className="py-2.5 px-4 text-center">
                    {has
                      ? <CheckCircle2 className="w-4 h-4 text-emerald-500 mx-auto" />
                      : <XCircle      className="w-4 h-4 text-gray-200 dark:text-neutral-700 mx-auto" />}
                  </td>
                ))}
              </tr>
            ))}
...
function InlineAuditPanel({ isManager, activeMerchantId }: { isManager: boolean; activeMerchantId: string | null }) {
  ...
  const filtered = logs.filter(log => {
    ...
    const safeUser = log.user_name || '';
    const safeTarget = log.target_name || '';
    const safeAction = log.action || '';

    const matchSearch = !q ||
      safeUser.toLowerCase().includes(q) ||
      safeTarget.toLowerCase().includes(q) ||
      safeAction.toLowerCase().includes(q);
      
    const matchFilter =
      filter === 'all'       ? true :
      filter === 'login'     ? safeAction === 'login' || safeAction === 'logout' :
      filter === 'user_'     ? safeAction.startsWith('user_') :
      filter === 'password_' ? safeAction.startsWith('password_') :
      true;
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 shadow-sm overflow-hidden">
        <div className="divide-y divide-gray-50 dark:divide-neutral-800 max-h-[480px] overflow-y-auto">
          {loading ? (
            ...
            filtered.map(log => {
              const safeAction = log.action || 'Unknown';
              const meta = AUDIT_META[safeAction] ?? { label: safeAction, cls: 'bg-gray-100 text-gray-600 dark:bg-neutral-800 dark:text-neutral-400' };
...
export function AcceptInvite({ currentUser }: UsersProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[Users] ERROR:', error.message, '| code:', error.code);
      }
      setUsers(data || []);
    } catch (err) {
  ...
  const filtered = users.filter(u => {
    ...
    return (
      (!q || safeName.toLowerCase().includes(q) || safeEmail.toLowerCase().includes(q) || safeRole.toLowerCase().includes(q)) &&
  ...
  const stats = {
    ...
    staff:       users.filter(u => isStaffRole(u.role || 'cashier')).length,
  };
